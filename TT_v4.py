...
def start():
    ...
    def create_env(config):
        ...
        binance_options = ExchangeOptions(
            commission=0.0075, min_trade_price=10.0
        )
        binance = Exchange(
            "binance", service=execute_order, options=binance_options
        )(p)
        ...
        kst_sig = ta.trend.KSTIndicator(
            close=taData[(coin + ":close")]
        ).kst_sig()
        dataset["kst_sig"] = kst_sig.pct_change()

        kst_diff = ta.trend.KSTIndicator(
            close=taData[(coin + ":close")]
        ).kst_diff()
        ...
        dataset["stc"] = ta.trend.STCIndicator(
            close=taData[(coin + ":close")]
        ).stc()
