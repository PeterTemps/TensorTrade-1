...
combined = open(
    os.path.join(newdirectory, "allcombined.txt"), "w"
)  # Creates a file of all the subs combined for dataset

for file in os.listdir(
    sys.argv[1]
):  # Gets All the files inside DIR (Argument1)

    newfile = open(
        os.path.join(newdirectory, file.replace(".vtt", ".txt")), "w"
    )  # Creates a newfile
